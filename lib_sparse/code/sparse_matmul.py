...
from code.triton_operators import unpack_batch_, unpack_relu2_batch_
...
def AspB(A: Tensor, B_sparse: BitsparseTensor, row_batch: int = 0) -> Tensor:
    ...
    if row_batch <= 0:
        ...
        dense = torch.empty(N, K, device=A.device, dtype=B_sparse.value_dtype)
    ...
    for first_n_tile in range(0, grid_m, row_tiles_per_batch):
        ...
        dense_batch = torch.empty(batch_rows, K, device=A.device, dtype=B_sparse.value_dtype)
...
def spAB(A_sparse: BitsparseTensor, B: Tensor, out: Tensor | None = None, row_batch: int = 0) -> Tensor:
    ...
    if row_batch <= 0:
        num_tiles = grid_m * grid_n
        dense = torch.empty(M, N, device=B.device, dtype=A_sparse.value_dtype)
    ...
    for first_m_tile in range(0, grid_m, row_tiles_per_batch):
        ...
        dense_batch = torch.empty(batch_rows, N, device=B.device, dtype=A_sparse.value_dtype)
